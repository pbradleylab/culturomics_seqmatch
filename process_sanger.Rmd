...
vsearch_results <- readr::read_tsv("./vsearch_output.txt",
...
interactive_alignment <- function(name="F_F01", trimmed_list=trimmed, vsearch=vsearch_processed) {
  ...
  aln_read_coords <- dplyr::filter(vsearch, qseqid==name) %>% dplyr::select(Species, qstart, qend)
  for (i in 1:nrow(aln_read_coords)) {
